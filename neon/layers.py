...
class FourierEnc(Module):
    ...
    def __call__(self, x):
        ...
        try:
          kernel = self.param('kernel', normal(self.embed_scale), (x.shape[-1], self.embed_dim // 2))
          y = jnp.dot(x, kernel)
        except IndexError: # x is a scalar, not array
          kernel = self.param('kernel', normal(self.embed_scale), (1, self.embed_dim // 2,))
          y = x*kernel.squeeze()
        y = jnp.concatenate([jnp.cos(y),
                             jnp.sin(y)], axis=-1)
        return y
